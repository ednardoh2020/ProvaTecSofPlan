...
begin
  if intThreadAtiva < StrToInt(EDT_NumThread.Text) then
    begin
      if Application.MessageBox('Thread ainda está rodando. Deseja Fechar assim mesmo ? ', 'Informação', MB_ICONQUESTION + MB_YESNO) = IDYES then
        Action:=cafree
      else
        Action:=canone;
    end;
